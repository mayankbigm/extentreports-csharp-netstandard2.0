...
namespace AventStack.ExtentReports.Core
{
    public abstract class ExtentObservable
    {
        ...
        /// <summary>
        /// Instance of <see cref="Core.ReportStatusStats"/>
        /// </summary>
        ...
        /// </summary>
        private readonly Dictionary<Status, bool> _statusMap = new Dictionary<Status, bool>();
        ...
        private void RefreshStatusList(List<Test> testList)
        {
            lock (_synclock)
            {
                ...
                foreach (var logStatus in distinctStatusList)
                {
                    if (!_statusMap.ContainsKey(logStatus))
                    {
                        _statusMap.Add(logStatus, false);
                    }
                }
        ...
        private void CopyContextInfoToObservable(Test test)
        {
            ...
            if (test.HasChildren)
            {
                test.NodeContext.All().ToList()
                    .ForEach(CopyContextInfoToObservable);
            }
        }
        ...
        private void NotifyReporters()
        {
            ...
            var reportAggregates = new ReportAggregates
            {
                ...
                ExceptionInfoContext = ExceptionInfoContext,
                TestRunnerLogs = TestRunnerLogs,
