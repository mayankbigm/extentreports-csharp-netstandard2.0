...
namespace AventStack.ExtentReports.Reporter
{
    public abstract class BasicFileReporter : ConfigurableReporter
    {
        ...
        protected void Initialize(BasicFileConfiguration userConfig)
        {
            ...
            IConfigurationRoot configurationDefault = new ConfigurationBuilder()
                .AddJsonFile("appsettings.json", optional: true, reloadOnChange: true)
                .Build();

            var config =configurationDefault.GetSection("HtmlReporter.UserSettings")
                .GetChildren()
                .Select(item => new Config(item.Key, item.Value)).ToList();

            foreach (Config item in config)
            {
                MasterConfig.AddConfig(item);
            }
        }
