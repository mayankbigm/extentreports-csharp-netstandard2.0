...
namespace AventStack.ExtentReports
{
    public class ExtentTest : IMediaContainer<ExtentTest>
    {
        ...
        public ExtentTest AssignAuthor(params string[] author)
        {
            ...
            author.Distinct().Where(c => !string.IsNullOrEmpty(c)).ToList().ForEach(a =>
            {
        ...
        public ExtentTest AssignCategory(params string[] category)
        {
            ...
            category.Distinct().Where(c => !string.IsNullOrEmpty(c)).ToList().ForEach(c =>
            {
        ...
        public ExtentTest AssignDevice(params string[] device)
        {
            ...
            device.Distinct().Where(c => !string.IsNullOrEmpty(c)).ToList().ForEach(a =>
            {
        ...
        /// <summary>
        /// Adds a screen shot using a file path
        /// </summary>
        ...
        private void AddScreenCapture(ScreenCapture screenCapture)
        {
            Model.ScreenCaptureContext.Add(screenCapture);

            int seq = Model.ScreenCaptureContext.Count;
            screenCapture.TestObjectId = Model.ObjectId;

            Extent.AddScreenCapture(Model, screenCapture);
        }

        /// <summary>
        /// Adds a screen shot using a base64 string
        /// </summary>
